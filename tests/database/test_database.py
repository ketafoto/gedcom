...
# Import scripts
IMPORT_SCRIPT = Path(__file__).parent / ".." / ".." / "database" / "gedcom_import.py"
EXPORT_SCRIPT = Path(__file__).parent / ".." / ".." / "database" / "gedcom_export.py"


class TestImportExportGedcom:
    """Test suite for GEDCOM 5.5.1 import/export roundtrip validation."""

    def test_roundtrip_import_export_consistency(self, log_test_step, test_db):
        ...
        result = subprocess.run(
            [sys.executable, str(IMPORT_SCRIPT), str(TEST_GEDCOM_FILE), str(TEST_DB_FILE)],
            capture_output=True,
            text=True
        )
        ...
        backup_files = list(TEST_DB_FILE.parent.glob(f"{TEST_DB_FILE}.*"))
        ...
        export_file1 = TEST_DB_FILE / "test_gedcom_export1.txt"
        result = subprocess.run(
            [sys.executable, str(EXPORT_SCRIPT), str(export_file1), str(TEST_DB_FILE)],
            capture_output=True,
            text=True
        )
        ...
        result = subprocess.run(
            [sys.executable, str(IMPORT_SCRIPT), str(export_file1), str(TEST_DB_FILE)],
            capture_output=True,
            text=True
        )
        assert result.returncode == 0, f"Second import failed: {result.stderr}"
        ...
        result = subprocess.run(
            [sys.executable, str(EXPORT_SCRIPT), str(export_file2), str(TEST_DB_FILE)],
            capture_output=True,
            text=True
        )
        assert result.returncode == 0, f"Second export failed: {result.stderr}"
        ...
        # Helper to normalize line endings and trailing spaces for comparison
