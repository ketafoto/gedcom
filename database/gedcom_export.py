...
def iso_to_gedcom_date(iso_date: Optional[str]) -> Optional[str]:
    ...
    Non-exact dates are stored in X_date_gedcom and used directly.
    """
    if not iso_date:
        return None
    ...
    try:
        ...
        with Session(bind=db_engine) as db:
            ...
            with open(output_file, "w", encoding="utf-8") as out:
                ...
                # File name
                out.write(f"1 FILE {output_file.name}\n")
                ...
                for ind in individuals:
                    ...
                    # Birth
                    if ind.birth_date_gedcom or ind.birth_date or ind.birth_place:
                        out.write("1 BIRT\n")
                        date_str = get_gedcom_date(ind.birth_date, ind.birth_date_gedcom)
                        if date_str:
                    ...
                    for event in ind.events:
                        ...
                        date_str = get_gedcom_date(event.event_date, event.event_date_gedcom)
                        if date_str:
                            out.write(f"2 DATE {date_str}\n")
                    ...
                    if ind.death_date_gedcom or ind.death_date or ind.death_place:
                        out.write("1 DEAT\n")
                        date_str = get_gedcom_date(ind.death_date, ind.death_date_gedcom)
                        if date_str:
                            out.write(f"2 DATE {date_str}\n")
                        if ind.death_place:
                ...
                for fam in families:
                    ...
                    if fam.marriage_date_gedcom or fam.marriage_date or fam.marriage_place:
                        out.write("1 MARR\n")
                        date_str = get_gedcom_date(fam.marriage_date, fam.marriage_date_gedcom)
                        if date_str:
                            out.write(f"2 DATE {date_str}\n")
                    ...
                    if fam.divorce_date_gedcom or fam.divorce_date:
                        out.write("1 DIV\n")
                        date_str = get_gedcom_date(fam.divorce_date, fam.divorce_date_gedcom)
                        if date_str:
                            out.write(f"2 DATE {date_str}\n")
