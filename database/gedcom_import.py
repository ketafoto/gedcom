...
def _create_individual(individual_data, db: Session) -> Individual:
    ...
    db_individual = Individual(
        ...
        birth_date_gedcom=individual_data.birth_date_gedcom,
        birth_place=individual_data.birth_place,
        death_date=individual_data.death_date,
        death_date_gedcom=individual_data.death_date_gedcom,
...
def _create_family(family_data, db: Session) -> Family:
    ...
    db_family = Family(
        ...
        marriage_date_gedcom=family_data.marriage_date_gedcom,
        marriage_place=family_data.marriage_place,
        divorce_date=family_data.divorce_date,
        divorce_date_gedcom=family_data.divorce_date_gedcom,
...
def parse_gedcom_date(gedcom_date: str) -> Optional[str]:
    ...
    For non-exact dates, store the raw GEDCOM string in X_date_gedcom instead.
    """
                ...
                if id_match:
                    ...
                    if record_type == "INDI":
                        current_record = {
                            ...
                            "names": [],
                            "sex": None,
                            "birth": {"date": None, "date_gedcom": None, "place": None},
                            "death": {"date": None, "date_gedcom": None, "place": None},
                        ...
                        current_record = {
                            ...
                            "wife": None,
                            "children": [],
                            "marr": {"date": None, "date_gedcom": None, "place": None},
                            "div": {"date": None, "date_gedcom": None, "place": None},
                            "events": [],   # Additional events
                            "media": [],    # Media objects
                            "notes": None
                        }
            ...
            if current_type == "INDI":
                if level == 1:
                    ...
                    if tag == "NAME":
                        ...
                        current_record["birth"] = {"date": None, "date_gedcom": None, "place": None}
                        current_event = current_record["birth"]
                        current_event["_tag"] = "BIRT"
                    elif tag == "DEAT":
                        current_record["death"] = {"date": None, "date_gedcom": None, "place": None}
                        ...
                        current_event = {"type": tag, "date": None, "date_gedcom": None, "place": None, "description": value or None}
                    ...
                    if current_event:
                        if tag == "DATE":
                            # Mutual exclusivity: exact dates go to date, others to date_gedcom
                            if is_exact_gedcom_date(value):
                                current_event["date"] = parse_gedcom_date(value)
                                current_event["date_gedcom"] = None
                            else:
                                current_event["date"] = None
                                current_event["date_gedcom"] = value
                        elif tag == "PLAC":
            ...
            # Family details
                ...
                if level == 1:
                    ...
                    if tag == "HUSB":
                        ...
                        current_record["marr"] = {"date": None, "date_gedcom": None, "place": None}
                        current_event = current_record["marr"]
                        current_event["_tag"] = "MARR"
                    elif tag == "DIV":
                        current_record["div"] = {"date": None, "date_gedcom": None, "place": None}
        ...
        with Session(bind=db_engine) as db:
            ...
            for gedcom_id, ind_data in individuals.items():
                ...
                ind_create = schemas.IndividualCreate(
                    ...
                    birth_date_gedcom=ind_data["birth"].get("date_gedcom"),
                    birth_place=ind_data["birth"]["place"],
                    death_date=ind_data["death"]["date"],
                    death_date_gedcom=ind_data["death"].get("date_gedcom"),
                ...
                for event_data in ind_data.get("events", []):
                    db_event = Event(
                        ...
                        event_date_gedcom=event_data.get("date_gedcom"),
            ...
            for gedcom_id, fam_data in families.items():
                fam_create = schemas.FamilyCreate(
                    ...
                    marriage_date_gedcom=fam_data["marr"].get("date_gedcom"),
                    marriage_place=fam_data["marr"]["place"],
                    divorce_date=fam_data["div"]["date"] if "div" in fam_data else None,
                    divorce_date_gedcom=fam_data["div"].get("date_gedcom") if "div" in fam_data else None,
